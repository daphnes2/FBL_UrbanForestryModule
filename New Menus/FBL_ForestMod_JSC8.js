...
const menuPanels = document.querySelectorAll('.tab-panel');
...
railButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
        ...
        // Clicking the currently-open tab's icon again closes it (collapses
        // back to icon-only rail); clicking a different icon switches to it.
        if (alreadyActive) {
            closeAllPanels();
        } else {
...
// Small × in each panel's header, per the prototype, so a panel can be
// closed without needing to re-click its rail icon.
