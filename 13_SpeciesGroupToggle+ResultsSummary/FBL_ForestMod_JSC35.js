...
async function getDistrictsData(city) {
    ...
    data.features.forEach((f) => { f.properties._bbox = computeGeometryBbox(f.geometry); });
...
// Builds the "<Group Type> Distribution" section as horizontal bars (the
// report's existing flat-color bar-chart pattern, same as Species and
// Diameter below) - for whichever classification is currently the
// UNTOGGLED one, shown below the donut section above.
function buildGroupDistributionBarSection(features, total, groupType) {
    const keys = groupKeysForType(groupType);
    const label = groupLabelForType(groupType);
    const counts = computeGroupCounts(features, groupType);
    ...
    keys.concat(['Other Species']).forEach((rowLabel, i) => {
        ...
        row.innerHTML = `
            <span class="report-bar-label" title="${escapeHtml(rowLabel)}">${escapeHtml(rowLabel)}</span>
...
function generateResultsSummary() {
    ...
    // --- Group distribution: donut for whichever classification the
    // Cluster Group Type toggle currently has selected, bar chart below
    // it for the other one - mirrors the map itself (map color/cluster
    // donuts/Legend all follow the same toggle). -----------------------
    const primaryGroupType = selectedClusterGroupType;
    const secondaryGroupType = primaryGroupType === 'building' ? 'eco' : 'building';

    body.appendChild(buildGroupDistributionDonutSection(filteredFeatures, total, primaryGroupType));
    body.appendChild(buildGroupDistributionBarSection(filteredFeatures, total, secondaryGroupType));
