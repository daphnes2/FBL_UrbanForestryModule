...
const speciesGroups = {
    ...
    "Group 4 (Versatile mid-weight hardwoods)": [
...
map.on('load', () => {
    ...
    // Render-throttling: as pages stream in we must NOT call setData()
    // on every page arrival. setData() on a *clustered* GeoJSON source
    ...
    // incremental/partial mode. The previous version only coalesced
    // calls within a single animation frame (~16ms), which is nowhere
    // near enough: with concurrency 8, dozens of pages can resolve over
    // the course of a several-second load, each one triggering a full
    // re-cluster of an ever-growing array. That repeated full-dataset
    // reclustering - not the raw data size alone - is what was crashing
    // the page around ~100k points. This instead enforces a minimum time
    // gap between renders (RENDER_THROTTLE_MS), so the map still fills
    // in progressively but only re-clusters a bounded number of times
    // total, with a trailing call guaranteeing the final state is never
    // dropped.
    ...
    fetchAllTrees((newFeatures, loaded, total, failedPages) => {
        ...
        // Also track every distinct SPECIES value seen, for the Individual
        // Species dropdown below.
...
// Confirmed field names: "name" holds the ward name, "ward" holds the
// ward number. Looked up case-insensitively as a defensive measure - if
// direct lowercase access (props.name) was returning "Unknown Ward" in
// testing, a casing mismatch in the actual GeoJSON (e.g. "Name" or
// "NAME") is the most likely cause, and this works regardless of which
// casing the service actually uses.
...
function getWardName(props) {
    const val = getPropCaseInsensitive(props, 'name');
    return val !== undefined ? val : 'Unknown Ward';
}
function getWardNumber(props) {
    const val = getPropCaseInsensitive(props, 'ward');
...
function addWardsLayers(data) {
    ...
    // Grayscale styling, per request (was blue).
    ...
    // of ward polygons instead of being hidden underneath them.
    map.addLayer({
        id: 'wards-fill',
        type: 'fill',
        source: 'wards',
    ...
    // Popup shows the ward name, with the ward number below it.
    map.on('click', 'wards-fill', (ev) => {
        const props = ev.features[0].properties;
        const wardName = getWardName(props);
        const wardNumber = getWardNumber(props);
        if (wardName === 'Unknown Ward') {
            // Should no longer trigger given case-insensitive lookup, but
            // left in as a safety net - if this fires, the real property
            // keys are printed here for a definitive fix.
            console.warn('Ward name not found - actual properties on this feature:', props);
        }
        const numberLine = wardNumber !== null ? `<p><b>Ward Number:</b> ${wardNumber}</p>` : '';
        new maplibregl.Popup()
            .setLngLat(ev.lngLat)
            .setHTML(`<h3>${wardName}</h3>${numberLine}`)
            .addTo(map);
    });
...
// so e.g. Info and Layers can both stay expanded while Species Building
// Groups stays collapsed. The whole rail + unified panel group pins to
// the actual left edge of the screen the moment anything is open (see
// updateMenuPinnedState() and .menu-wrap.pinned in the CSS) - same
// icons/fonts/card styling as the floating default, just anchored
// differently once "activated". Each section can be toggled either from
// its rail icon or by clicking its own header inside the panel - both
// paths go through the same toggleSection()/openPanel()/closePanel().
//===================================================================
const railButtons = document.querySelectorAll('.rail-btn');
const sectionHeaders = document.querySelectorAll('.section-header');
const menuWrapEl = document.querySelector('.menu-wrap');
const unifiedPanelEl = document.getElementById('unifiedPanel');
const titleCardEl = document.querySelector('.title-card');

// The title card's width is auto-sized to its content (logo + text),
// not a fixed value, so matching it requires measuring the actual
// rendered width in JS rather than a CSS rule - .unified-panel's CSS
// width acts only as a fallback until this runs. Re-measured on window
// resize in case font loading or layout shifts change the title card's
// width after first paint.
function syncPanelWidthToTitleCard() {
    const width = titleCardEl.getBoundingClientRect().width;
    if (width > 0) {
        unifiedPanelEl.style.width = `${width}px`;
    }
}
syncPanelWidthToTitleCard();
window.addEventListener('load', syncPanelWidthToTitleCard);
window.addEventListener('resize', syncPanelWidthToTitleCard);

function updateMenuPinnedState() {
    const anyOpen = document.querySelector('.panel-section.active') !== null;
    menuWrapEl.classList.toggle('pinned', anyOpen);
    unifiedPanelEl.classList.toggle('open', anyOpen);
}

function openPanel(tabName) {
    const panel = document.getElementById(`panel-${tabName}`);
    const btn = document.querySelector(`.rail-btn[data-tab="${tabName}"]`);
...
railButtons.forEach((btn) => {
    btn.addEventListener('click', () => toggleSection(btn.dataset.tab));
});

// Clicking a section's own header inside the panel also toggles it -
// same as clicking its rail icon (matches the reference layout's
// per-section –/⌄ header controls).
sectionHeaders.forEach((header) => {
    header.addEventListener('click', () => toggleSection(header.dataset.tab));
});

// Starts fully collapsed (icon rail only, no panel open) to match the
// prototype's default/idle state.
...
// the coarser Species Group filter above. Empty selection = no filtering.
...
//
// Summarizes whatever the CURRENT filters (Species Group, Individual
// Species, Planting Conditions, Diameter) produce - it reuses
...
// --- Embodied carbon assumptions (all deliberately simple/adjustable -
// see the methodology note rendered in the report itself) ---------
// Source: ICE (Inventory of Carbon & Energy) database - "Timber,
// average of all data, no carbon storage" = 0.493 kgCO2e per kg of
// timber.
const TIMBER_CARBON_FACTOR_KG_CO2E_PER_KG = 0.493;
...
// midpoint-ish placeholder, not a precise ICE-sourced value. Adjust
// if you have a more specific density in mind.
const ASSUMED_WOOD_DENSITY_KG_PER_M3 = 500;
// The dataset has no tree-height field, and rigorous volume estimation
// normally needs height + taper equations, which is well beyond a
// "rough estimate". Every tree's trunk is instead modeled as a simple
// cylinder using its own DBH, but ONE constant assumed height for
// every tree (this is what makes it "based on tree count and
// diameter" rather than requiring real height data).
const ASSUMED_TREE_HEIGHT_M = 10;

// Returns { totalKgCO2e, treesUsed, treesExcludedNoDBH }
function computeEmbodiedCarbonEstimate(features) {
    ...
    features.forEach((f) => {
        ...
        if (dbhCm === null || dbhCm === undefined || dbhCm === '' || isNaN(Number(dbhCm))) {
            treesExcludedNoDBH += 1;
            return;
        }
        const radiusM = (Number(dbhCm) / 100) / 2; // cm diameter -> m radius
        const volumeM3 = Math.PI * radiusM * radiusM * ASSUMED_TREE_HEIGHT_M;
    ...
    return { totalKgCO2e, treesUsed, treesExcludedNoDBH };
}
...
function generateResultsSummary() {
    ...
    // --- Building Group distribution (donut, reusing map colors) ----
    const groupCounts = {};
    for (let g = 1; g <= 8; g++) groupCounts[g] = 0;
    filteredFeatures.forEach((f) => {
        const gid = f.properties._groupId || 8;
        groupCounts[gid] = (groupCounts[gid] || 0) + 1;
    });

    const groupTitle = document.createElement('div');
    groupTitle.className = 'report-section-title';
    groupTitle.textContent = 'Building Group Distribution';
    body.appendChild(groupTitle);

    const donutRow = document.createElement('div');
    donutRow.className = 'report-donut-row';
    const donut = buildReportDonutSVG(groupCounts, total, 110);
    donutRow.appendChild(donut);

    const groupLegend = document.createElement('div');
    groupLegend.className = 'report-group-legend';
    speciesGroupKeys.concat(['Other Species']).forEach((label, i) => {
        const groupId = i + 1;
        ...
        row.innerHTML = `<span class="legend-swatch round" style="background:${GROUP_COLORS[groupId]};"></span> ${escapeHtml(label)} <span style="color:#999; margin-left:auto; flex-shrink:0; white-space:nowrap;">${count} (${pct}%)</span>`;
        row.style.display = 'flex';
        row.style.alignItems = 'center';
        row.style.gap = '6px';
        groupLegend.appendChild(row);
    });
    donutRow.appendChild(groupLegend);
    ...
    const excludedNote = carbon.treesExcludedNoDBH > 0
        ? `<p>${carbon.treesExcludedNoDBH.toLocaleString()} of ${total.toLocaleString()} trees have no recorded diameter and were excluded from this estimate (based on ${carbon.treesUsed.toLocaleString()} trees).</p>`
        : '';

    const methodology = document.createElement('div');
    methodology.className = 'report-methodology';
    methodology.innerHTML = `
        <p><b>Methodology (rough estimate):</b></p>
        <p>Each tree's trunk is modeled as a simple cylinder using its recorded diameter (DBH) and one assumed height of ${ASSUMED_TREE_HEIGHT_M}m for every tree (no height data exists in this dataset). Volume &times; an assumed wood density of ${ASSUMED_WOOD_DENSITY_KG_PER_M3} kg/m&sup3; gives mass; mass &times; ${TIMBER_CARBON_FACTOR_KG_CO2E_PER_KG} kgCO2e/kg gives embodied carbon.</p>
        <p>Carbon factor source: ICE (Inventory of Carbon &amp; Energy) database - "Timber, average of all data, no carbon storage".</p>
        ${excludedNote}
        <p>This is a rough, illustrative estimate only - not a substitute for species-specific volume tables or a certified carbon assessment.</p>
    `;
