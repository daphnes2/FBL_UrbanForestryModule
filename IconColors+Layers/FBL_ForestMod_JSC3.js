...
// Cluster bubble shading by tree count - lighter green = fewer trees,
// darker green = more trees. Defined once here and reused for both the
// cluster paint expression and the Legend tab, so they can never drift
// out of sync with each other.
const CLUSTER_COLOR_STEPS = [
    { color: '#c5e8c5', label: 'Fewer than 50 trees' },
    { color: '#6fae6f', threshold: 50, label: '50–199 trees' },
    { color: '#2f6b2f', threshold: 200, label: '200+ trees' }
];
...
map.on('load', () => {
    ...
    // Cluster bubbles - shading built from CLUSTER_COLOR_STEPS (see above)
    // so the map and the Legend tab always agree with each other.
    map.addLayer({
        id: 'clusters',
        type: 'circle',
        source: 'o_trees',
        ...
        paint: {
            'circle-color': [
                ...
                CLUSTER_COLOR_STEPS[0].color,
                CLUSTER_COLOR_STEPS[1].threshold, CLUSTER_COLOR_STEPS[1].color,
                CLUSTER_COLOR_STEPS[2].threshold, CLUSTER_COLOR_STEPS[2].color
            ],
            'circle-radius': [
                'step', ['get', 'point_count'],
                16, 50,
...
layerTreesToggle.addEventListener('change', (e) => {
    ...
    whenMapReady(() => {
        ['clusters', 'cluster-count', 'o_trees'].forEach((id) => {
...
// / CLUSTER_COLOR_STEPS constants used elsewhere, so it can't drift out
// of sync with what's actually drawn on the map)
//===================================================================
...
CLUSTER_COLOR_STEPS.forEach((step) => {
    const row = document.createElement('div');
    row.className = 'legend-row';
    row.innerHTML = `<span class="legend-swatch round" style="background:${step.color};"></span> ${step.label}`;
    legendClusterEl.appendChild(row);
});
