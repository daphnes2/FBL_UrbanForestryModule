...
// speciesGroups/UNG_TAPER_COEFFICIENTS entry) - genus+species,
...
// only falls back to the English word-matching (computeSpeciesGroupId
// against props.SPECIES) when no botanical name exists at all.
function computeSpeciesGroupIdForTree(props) {
    if (props._botanicalName) {
        const norm = normalizeLatinBinomial(props._botanicalName);
        for (let i = 0; i < speciesGroupKeys.length; i++) {
            if (matchesGroupByLatin(norm, speciesGroups[speciesGroupKeys[i]])) return i + 1;
        }
        return 8; // botanical name present but matches none of the 7 groups - Other Species
    }
    return computeSpeciesGroupId(props.SPECIES);
}
...
function normalizeTorontoRecord(rec) {
    ...
    return {
        ...
        properties: {
            ...
            _botanicalName: rec.BOTANICAL_NAME || null, // primary species-matching key (see computeSpeciesGroupIdForTree)
...
function loadTreesForCity(city) {
    ...
    function handlePage(rawItems, loaded, total, failedPages) {
        ...
        newFeatures.forEach((f) => {
            f.properties._groupId = computeSpeciesGroupIdForTree(f.properties);
...
map.on('click', 'o_trees', (e) => {
    ...
    // actually drove the species-group match, per computeSpeciesGroupIdForTree).
...
//===================================================================
// Uses each tree's precomputed _groupId (set via computeSpeciesGroupIdForTree
...
// can never disagree with what _groupId already decided (map color,
// legend, carbon report).
function passesSpeciesFilter(props) {
    if (selectedSpeciesGroup === '__other__') {
        return props._groupId === 8;
    }
    if (selectedSpeciesGroup === 'all') return true;
    const idx = speciesGroupKeys.indexOf(selectedSpeciesGroup);
    return props._groupId === idx + 1;
}
...
// computeSpeciesGroupIdForTree above, applied to the taper-coefficient
...
function getUngTaperCoefficientsForTree(props) {
    if (props._botanicalName) {
        ...
        return found || null; // no fallback to English word-matching - see comment above computeSpeciesGroupIdForTree
    }
